...
interface Reminder {
  id: string;
  type: 'temperature' | 'supplement' | 'appointment' | 'prenatal' | 'hydration' | 'other';
  title: string;
  subtitle: string;
  time: string;
  isCompleted?: boolean;
...
const recentNotifications: Notification[] = [
  {
    ...
    description: 'Your peak fertility starts tomorrow. Consider tracking cervical mucus.',
    time: '2h ago',
    isRead: false,
    priority: 'high',
  ...
  },
  {
    ...
    description: 'New personalized insights ready from this week\'s data. View personalized insights.',
    time: '4h ago',
    isRead: false,
    priority: 'medium',
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-100 to-pink-200 p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        {/* Header */}
        <div>
          <h1 className="text-2xl font-semibold text-gray-800">Reminders & Notifications</h1>
        </div>
              ...
              <CardHeader className="pb-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <CardTitle className="text-lg font-semibold text-gray-800">Active Reminders</CardTitle>
                  </div>
                ...
                {activeReminders.map((reminder) => (
                  <div key={reminder.id} className="flex items-center gap-4 p-4 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-sm text-gray-900">{reminder.title}</h3>
                    ...
                    <div className="flex items-center gap-2">
                      {reminder.status === 'completed' ? (
                        ...
                        <div className="flex items-center gap-2">
                          <Button size="sm" className="h-8 w-8 p-0 bg-green-100 hover:bg-green-200 rounded-full">
                            <CheckCircle className="w-4 h-4 text-green-600" />
                          </Button>
                          <Button size="sm" className="h-8 w-8 p-0 bg-orange-100 hover:bg-orange-200 rounded-full">
                            <Clock className="w-4 h-4 text-orange-600" />
                          </Button>
                          <Button size="sm" className="h-8 w-8 p-0 bg-blue-100 hover:bg-blue-200 rounded-full">
              ...
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <Badge variant="outline" className="bg-red-50 text-red-600 border-red-200 text-xs px-2 py-1">
                      12 unread
                    </Badge>
                    <Badge variant="outline" className="bg-blue-50 text-blue-600 border-blue-200 text-xs px-2 py-1">
                      47 this week
                    </Badge>
                  </div>
                </div>
              ...
              <CardContent className="space-y-3">
                {recentNotifications.map((notification) => (
                  <div key={notification.id} className={`p-3 rounded-lg border transition-colors ${
                    notification.isRead ? 'border-gray-100 bg-gray-50' : 'border-blue-100 bg-blue-50'
                  }`}>
                    <div className="flex items-start gap-2">
                      <div className={`p-1 rounded-full ${notification.isRead ? 'bg-gray-200' : 'bg-blue-100'} ${
                        getPriorityColor(notification.priority, notification.isRead)
                      }`}>
                        {notification.icon}
                      </div>
                      <div className="flex-1 min-w-0">
                        <h4 className={`font-medium text-xs ${notification.isRead ? 'text-gray-700' : 'text-gray-900'}`}>
                          {notification.title}
                        </h4>
            ...
            <Tabs defaultValue="schedule" className="w-full">
              <div className="border-b px-6 pt-6">
                <TabsList className="grid w-full max-w-md grid-cols-3 bg-gray-100">
                  <TabsTrigger value="schedule" className="data-[state=active]:bg-white data-[state=active]:text-pink-600">
                    Reminder Schedule
                  </TabsTrigger>
                  <TabsTrigger value="history" className="data-[state=active]:bg-white data-[state=active]:text-pink-600">
                    Notification History
                  </TabsTrigger>
                  <TabsTrigger value="settings" className="data-[state=active]:bg-white data-[state=active]:text-pink-600">
                      ...
                      <Card key={item.id} className="border border-gray-200 hover:shadow-md transition-shadow">
                        <CardContent className="p-4">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-xs font-medium text-gray-900">{item.time.split(' - ')[0]}</span>
                          </div>
              ...
              <TabsContent value="history" className="p-6">
                <div className="text-center py-12">
                  ...
                  <h3 className="font-semibold text-gray-900 mb-2">Notification History</h3>
                  <p className="text-sm text-gray-600 max-w-md mx-auto">View your complete notification timeline and archive. Track all past reminders and notifications.</p>
                </div>
              </TabsContent>
              ...
              <TabsContent value="settings" className="p-6">
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-gray-600 max-w-md mx-auto">Customize your reminder and notification settings. Control when and how you receive alerts.</p>
                </div>
              </TabsContent>
            </Tabs>
          </CardContent>
