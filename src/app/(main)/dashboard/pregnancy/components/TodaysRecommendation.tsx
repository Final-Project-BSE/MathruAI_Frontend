...
function getCardTitleFromText(text: string, index: number): string {
...
const TodaysRecommendation: React.FC<TodaysRecommendationProps> = ({
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        ...
        if (!jwt) {
          setError('Please log in to view today’s recommendation.');
          return;
        }
        ...
        if (!uid) {
          setError('Authenticated, but user ID could not be resolved.');
          return;
        }
  ...
  const previewItems = useMemo(() => {
    const parsed = parseRecommendationToItems(recommendation?.recommendation || '');
          ...
          <div className="rounded-2xl border border-[#f3c7c8] bg-[#fff5f5] p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-[#8a4b4c]">{error}</p>
              </div>
            </div>
          </div>
        </CardContent>
              ...
              {previewItems.length > 0 ? (
                    ...
                    {marqueeItems.map((item, index) => (
                      ...
                      >
                        <h4 className="mb-2 text-sm font-semibold text-[#d04f51]">
                          {getCardTitleFromText(item, index % previewItems.length)}
                        </h4>
                ...
                <div className="rounded-xl border border-[#d04f51]/20 bg-[#d04f51]/5 p-4">
                  <p className="text-sm leading-6 text-gray-700">
                    {recommendation.recommendation || 'No recommendation available yet.'}
                  </p>
                </div>
              )}
