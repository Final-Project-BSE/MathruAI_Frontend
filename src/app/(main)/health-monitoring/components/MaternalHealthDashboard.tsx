...
const MaternalHealthDashboard = () => {
  ...
  useEffect(() => {
    ...
    void initialize();
  }, [text.loginAccessPage, text.authError]);
  ...
  const getRequiredFieldLabel = (field: keyof VitalsState): string => {
    return text.profile.fields[field] || String(field);
  };

  const handlePrediction = async () => {
    if (!token || !isAuthenticated) {
      setError(text.loginSavePredictions);
      return;
    }
  ...
  const handleRefresh = () => {
    if (token) void loadSavedData(token);
  };
