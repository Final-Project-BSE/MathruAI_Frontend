...
const ChangeRoleCard = ({ profile, token, userId, onUpdate }: Props) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedRole) {
      errorToast('Please select a stage.');
      return;
    }
    ...
    try {
      ...
      actionLogoutToast({
        ...
        description: (
          <>
            You have upgraded to <strong>"{stageName}"</strong>.
            <br />
            Please log out and sign in again as <strong>"{stageName}"</strong>.
          </>
        ),
        confirmText: 'Logout',
        cancelText: 'Close',
      ...
      errorToast(err.message || 'Failed to update stage');
    } finally {
      setLoading(false);
    }
  };
