...
export default function PatientAppointmentsCard({
  ...
  const refreshBookedSlots = useCallback(async (dateOverride?: string | null) => {
  ...
  const visibleAppointments = useMemo(() => {
    return showAllAppointments ? currentAppointments : currentAppointments.slice(0, 2);
  ...
  async function handleCreate(payload: AppointmentCreateRequestDto) {
    try {
      ...
      if (err instanceof Error && err.message === "Selected time slot is already booked.") {
  ...
  async function handleDeleteCompleted(row: AppointmentResponseDto) {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to delete completed appointment."
      );
    } finally {
      setActionLoadingId(null);
    }
