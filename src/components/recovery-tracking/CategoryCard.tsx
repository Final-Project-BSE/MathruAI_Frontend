...
import { RecoveryTask, CATEGORY_LABELS, CATEGORY_COLORS, TaskCategory } from "./recovery-data";
        ...
        >
          ...
          <div className="space-y-2">
            {tasks.map((task) => {
              ...
              return (
                <label
                  key={task.id}
                  className={`flex items-start gap-3 p-3 rounded-xl cursor-pointer transition-all ${isWarning
                ...
                >
                  ...
                  <span className={`text-sm flex-1 pt-0.5 ${isCompleted && !isWarning ? ' text-gray-600' : isWarning ? 'text-red-900 font-medium' : 'text-gray-700 font-medium'
                    }`}>
                    {task.text}
                  </span>
          ...
          {isWarning && (
            <div className="mt-4 p-3 bg-red-100 rounded-xl flex items-center justify-between border border-red-200">
              ...
              <button type="button" onClick={() => setMessagesOpen(true)} className="px-4 py-1.5 bg-red-600 text-white text-xs font-bold rounded-full shadow-sm hover:bg-red-700 transition-colors">
                Contact Midwife
              </button>
            </div>
          )}
