...
export default function RecordFormModal({
  ...
  const { t } = useLanguage();
  const form = t.healthRecords.form;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!name.trim()) newErrors.name = form.recordNameRequired;
    if (!date) newErrors.date = form.dateRequired;
      ...
      <DialogContent className="sm:max-w-[520px] rounded-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-[#d04f51] text-base font-semibold">
            {initialData ? form.editRecord : form.addNewRecord}
          </DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-2">
          <div className="space-y-1.5">
            <Label htmlFor="rec-name" className="text-sm font-medium text-gray-700">
              {form.recordName} <span className="text-red-500">*</span>
            </Label>

            <Input
              id="rec-name"
              placeholder={form.recordNamePlaceholder}
              value={name}
              onChange={(e) => {
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <p className="text-xs text-gray-400">{form.supports}</p>
            </div>
          ...
          <DialogFooter className="gap-2 pt-2">
            ...
            <Button
              type="submit"
              className="flex-1 rounded-xl bg-[#d04f51] text-white"
            >
              {initialData ? form.saveChanges : form.addRecord}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
