  ...
  const loadChatSessions = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const normalizedSessions = ((data.sessions || []) as ApiChatSession[]).reduce<
          ChatSession[]
        >((acc, session) => {
        ...
        console.error(
          'Failed to load chat sessions - Response not OK:',
          response.status
        )
      }
    } catch (error) {
          ...
          prev.map((msg) =>
            msg.message_id === tempUserMessage.message_id
              ? {
                ...
                response_time_ms: data.processing_time_seconds * 1000,
              }
              : msg
          )
        )
  ...
  const ChatSidebar = ({
    ...
    <div
      className={`${isOpen ? 'w-64' : 'w-0'
        } transition-all duration-300 overflow-hidden bg-gradient-to-b from-pink-50 to-white border-r border-pink-200`}
    >
          ...
          {sessions.length === 0 ? (
            ...
            sessions.map((session: ChatSession) => (
              <div
                ...
                className={`p-3 rounded-lg mb-2 cursor-pointer transition-all duration-200 group border-b border-pink-200 ${currentSessionId === session.session_id
